...
struct hf22_database_fixture : public clean_database_fixture_wrap {
    hf22_database_fixture() : clean_database_fixture_wrap(true, [&]() {
        // WARNING: we cannot use fixture class fields here!
        // Because they are not yet initialized
        initialize();
        open_database();
        startup_with_hf(hf);
    }) {
    ...
    void resize_shared_mem(uint64_t size, bool restart = true) {
        _db.wipe(data_dir->path(), data_dir->path(), true);
        _db.open(data_dir->path(), data_dir->path(), INITIAL_TEST_SUPPLY, size, chainbase::database::read_write);
...
BOOST_FIXTURE_TEST_SUITE(hf22_tests, hf22_database_fixture)
    ...
    BOOST_AUTO_TEST_CASE(hf22_witness_vote_staked) { try {
        ...
        BOOST_CHECK_GT(_db.get_witness(w[0]).votes, _db.get_witness(w[1]).votes);

        auto a0_weight = _db.get_account(a[0]).vesting_shares.amount.value;
        auto a1_weight = _db.get_account(a[1]).vesting_shares.amount.value;
        BOOST_CHECK_EQUAL(_db.get_witness(w[0]).votes, a0_weight + a1_weight);
        BOOST_CHECK_EQUAL(_db.get_witness(w[2]).votes, a0_weight);
        BOOST_CHECK_EQUAL(_db.get_witness(w[3]).votes, a0_weight);
        BOOST_CHECK_EQUAL(_db.get_witness(w[4]).votes, a1_weight);
        BOOST_CHECK_EQUAL(_db.get_witness(w[5]).votes, a1_weight);

        BOOST_TEST_MESSAGE("-- Retally votes");

        _db.set_hardfork(STEEMIT_HARDFORK_0_22);
        generate_block();
        validate_database();

        BOOST_TEST_MESSAGE("-- Checking votes");

        BOOST_CHECK_LT(_db.get_witness(w[0]).votes, _db.get_witness(w[1]).votes);

        a0_weight = _db.get_account(a[0]).vesting_shares.amount.value / 3;
        a1_weight = _db.get_account(a[1]).vesting_shares.amount.value / 3;
        BOOST_CHECK_EQUAL(_db.get_witness(w[0]).votes, a0_weight + a1_weight);
        BOOST_CHECK_EQUAL(_db.get_witness(w[2]).votes, a0_weight);
        BOOST_CHECK_EQUAL(_db.get_witness(w[3]).votes, a0_weight);
        BOOST_CHECK_EQUAL(_db.get_witness(w[4]).votes, a1_weight);
    ...
    BOOST_AUTO_TEST_CASE(hf22_witness_idleness) { try {
        ...
        BOOST_CHECK_EQUAL(_db.get_witness(w[0]).votes, 0);
        BOOST_CHECK_EQUAL(_db.get_witness(w[2]).votes, 0);
        BOOST_CHECK_EQUAL(_db.get_witness(w[3]).votes, 0);
        BOOST_CHECK_EQUAL(_db.get_witness(w[4]).votes, 0);
        BOOST_CHECK_EQUAL(_db.get_witness(w[5]).votes, 0);
    } FC_LOG_AND_RETHROW() }
    ...
    BOOST_AUTO_TEST_CASE(hf22_sbd_debt_conversion) { try {
        ...
        BOOST_CHECK_EQUAL(_db.get_account("dave").sbd_balance.amount.value, 0);
        validate_database();

        auto dave_payout_time = _db.get_comment_by_perm("dave", string("test")).cashout_time;
        generate_blocks(dave_payout_time, true);
        BOOST_CHECK_GT(_db.get_account("dave").sbd_balance.amount.value, 0);

        transfer<transfer_to_savings_operation>("dave", "dave", _db.get_account("dave").sbd_balance / 2);

        BOOST_TEST_MESSAGE("-- Making pending interest payout");

        transfer("dave", "dave2", asset(2000, SBD_SYMBOL));

        generate_blocks(_db.head_block_time() + fc::days(90));
        ...
        BOOST_CHECK_EQUAL(_db.get_dynamic_global_properties().sbd_debt_percent, 0);
        validate_database();

        set_price_feed(price(asset::from_string("100000.000 GOLOS"), asset::from_string("1.000 GBG")));

        {
            auto gb = STEEMIT_FEED_INTERVAL_BLOCKS - _db.head_block_num() % STEEMIT_FEED_INTERVAL_BLOCKS;
            generate_blocks(gb);
        }
        ...
        BOOST_CHECK_GE(_db.get_dynamic_global_properties().sbd_debt_percent, STEEMIT_SBD_DEBT_CONVERT_THRESHOLD);
        ...
        {
            auto gb = STEEMIT_SBD_DEBT_CONVERT_INTERVAL - _db.head_block_num() % STEEMIT_SBD_DEBT_CONVERT_INTERVAL;
    ...
    BOOST_AUTO_TEST_CASE(hf22_account_idleness) { try {
        ...
        generate_blocks(_db.head_block_time() + fc::days(6*30));
        ...
        {
            auto gb = GOLOS_ACCOUNT_IDLENESS_CHECK_INTERVAL - _db.head_block_num() % GOLOS_ACCOUNT_IDLENESS_CHECK_INTERVAL;
            generate_blocks(gb);
        }

        BOOST_CHECK_EQUAL(_db.get_account("alice").to_withdraw, _db.get_account("alice").vesting_shares.amount);
        BOOST_CHECK_EQUAL(_db.get_account("bob").to_withdraw, _db.get_account("bob").vesting_shares.amount);
        BOOST_CHECK_EQUAL(_db.get_account("carol").to_withdraw, _db.get_account("carol").vesting_shares.amount);
