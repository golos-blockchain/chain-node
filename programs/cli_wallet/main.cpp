...
int unsafe_main(int argc, char** argv) {
    ...
    opts.add_options()
            ...
            ("server-rpc-endpoint,s", bpo::value<string>()->implicit_value("ws://127.0.0.1:8091"), "Server websocket RPC endpoint")
            ("cert-authority,a", BPO_VALUE_DEFAULT(string, "_default"), "Trusted CA bundle file for connecting to wss:// TLS server")
            ("rpc-endpoint,r", bpo::value<string>()->implicit_value("127.0.0.1:8092"), "Endpoint for wallet websocket RPC to listen on")
            ("rpc-tls-endpoint,t", bpo::value<string>()->implicit_value("127.0.0.1:8093"), "Endpoint for wallet websocket TLS RPC to listen on")
            ("rpc-tls-certificate,c", BPO_VALUE_DEFAULT(string, "server.pem"), "PEM certificate for wallet websocket TLS RPC")
            ("rpc-http-endpoint,H", bpo::value<string>()->implicit_value("127.0.0.1:8094"), "Endpoint for wallet HTTP RPC to listen on")
